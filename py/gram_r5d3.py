...
t32=map(lambda i: (numpy.array([int(x0==i[0] and x1==i[1] and x2==i[2] and x3==i[3] and x4==i[4]) for x0 in n for x1 in n for x2 in n for x3 in n for x4 in n]).reshape(3,3,3,3,3),[0,1,2,3,4]),e5)
...
t80=[(a,s) for a in map(lambda i: numpy.array([int(x0==i[0] and x1==i[1] and x2==i[2])*kr(x3,x4) for x0 in n for x1 in n for x2 in n for x3 in n for x4 in n]).reshape(3,3,3,3,3),e3) for s in axes10]
...
t30=[(a,s) for a in map(lambda i: numpy.array([int(x0==i)*kr(x1,x2)*kr(x3,x4) for x0 in n for x1 in n for x2 in n for x3 in n for x4 in n]).reshape(3,3,3,3,3),[0,1]) for s in axes15]
...
#numpy.savetxt("/tmp/monom_4.txt",m,fmt="%u")
